...
function AdminPage() {
  ...
  const assign = useMutation({
    mutationFn: () => {
      if (!assignTarget) throw new Error("No target");
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Grant authority or admin access, and assign authorities to departments.</p>
      </div>
                ...
                <div key={u.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {u.email}
                      {u.profile?.department && <> · {labelOf(DEPARTMENTS, u.profile.department)}</>}
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    {u.roles.map((r) => (
                      <Badge key={r} variant={r === "admin" ? "default" : "outline"} className="gap-1 capitalize">
                        ...
                        {r}
                        <button className="ml-1 opacity-60 hover:opacity-100" onClick={() => revoke.mutate({ userId: u.id, role: r })}>
                          <X className="h-3 w-3" />
                        </button>
                      </Badge>
                    ))}
                    <Button size="sm" variant="outline" onClick={() => setAssignTarget(u)}>Assign role</Button>
                  </div>
                </div>
              ))}
            </div>
